...
def core_cnn_layers(img_in, drop, l4_stride=1):
    ...
    x = Dropout(drop)(x)
    x = Conv2D(128, 3, l4_stride, padding='same')(x)
    x = Dropout(drop)(x)
    x = Flatten(name='flattened')(x)
    return x

def default_n_linear(num_outputs, num_road_sign_classes, input_shape=(120, 160, 3)):
    drop = 0.2
    img_in = Input(shape=input_shape, name='img_in')
    x = core_cnn_layers(img_in, drop)
    x = Dense(256, activation='relu', name='dense_1')(x)
    ...
    road_sign_output = Dense(num_road_sign_classes, activation='softmax', name='road_sign_output')(x)
    outputs.append(road_sign_output)
